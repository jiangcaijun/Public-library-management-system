					...
					<div id="content">
						...
						<div class="formbody">
							...
							<div id="usual1" class="usual">
								...
								<div class="itab">
									<ul>
										<li><a href="#tab111" class="selected">图书续借</a></li>

									</ul>
								</div>
								<div id="tab1" class="tabson">
									<form:form method="post" action="/front/tabReBorrow"
										modelAttribute="frontTabBook">
										<ul class="seachform">
											<li><label>当前正在借阅的书籍&nbsp;<input name="bookId"id="bookId" type="text" class="dfinput" style="width: 308px;" /></li>
										...
										<c:forEach var="items" items="${borrows}">
											<tbody>
												...
												<c:set var="nowDate" value="<%=System.currentTimeMillis()%>"></c:set> 
													...
													<c:if test="${nowDate-items.willReturnTime.time > 0}"> 
														<td><span style="color: red">已超期</span></td>
													</c:if>
													<c:if test="${nowDate-items.willReturnTime.time < 0}">
														<td>未超期</td>
													</c:if> 
													...
													<c:if test="${items.status > 0}">
														<td><span style="color: red">已续借</span></td>
													</c:if>
													
													<c:if test="${(nowDate-items.willReturnTime.time > 0)||(items.status > 0)}"> 
														<td><span>不可续借</span></td>
													</c:if>
													<c:if test="${(nowDate-items.willReturnTime.time < 0)&&(items.status < 1)}">
														<td><a href="/front/reBorrow2?bookId=${items.bookId}" class="tablelink">
															可续借</a></td>
													</c:if>
